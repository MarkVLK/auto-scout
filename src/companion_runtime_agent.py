...
def run_heartbeat(site_path=None):
    try:
        ...
        raise SystemExit("rospy is required to run the companion runtime agent: {}".format(exc))

    site_config, resolved_site_path = load_site_config(site_path)
    role_settings = role_config(site_config, "companion")
    payload = {
        "role": "companion",
    ...
    rospy.init_node("companion_runtime_agent", anonymous=False)
    publisher = rospy.Publisher("/companion/runtime_status", String, queue_size=1, latch=True)
    rate = rospy.Rate(0.5)
