...
Auto-Scout is a companion-first autonomy project for a Moorebot Scout with an added LD19-class 2D LiDAR. The current default is a no-LD19 holding mode while the sensor is detached for a permanent mount/harness, so Scout-local safety and vendor/manual behavior stay available without launching scan-dependent autonomy.
...
- Publish or bridge camera, IMU, ToF, and LD19 scan data when the LD19 is physically attached.
...
- Keep the deploy-managed `/userdata/auto-scout/auto-scout.swap` file active across reboot to absorb short memory spikes on the constrained Scout.
...
- Run `slam_gmapping`, `map_server`, `amcl`, `move_base`, and optional `explore_lite` only after `/scan` and pose are restored.
...
- `launch/scout_runtime.launch`
  Scout-side launch for the thin bridge runtime, including optional direct camera, optional LD19, ToF, IMU, odometry, low-battery dock guard, safety filtering, and motion normalization
- `launch/companion_runtime.launch`
  companion-side launch for the runtime heartbeat and lazy vendor JPG bridge, with localization, SLAM, and map-return gated behind `enable_nav_stack`
...
2. Hold scan-dependent autonomy until the LD19 is mounted:
   with the current defaults, `AUTO_SCOUT_ENABLE_LIDAR=false` and `AUTO_SCOUT_ENABLE_NAV_STACK=false`; after `/scan` returns, prove scan plus pose before treating `move_base` as ready
...
4. Start with manual-drive mapping after `/scan` is restored:
...
|   |-- BRINGUP_ROADMAP.md                              # Staged plan for validating the device from code fixes through missions.
|   |-- CODE_REVIEW_2026-08.md                          # August 2026 review findings covering bugs, security, and doc accuracy.
...
|   |-- ROCKCHIP_SERVICE_FINDINGS.md                    # Findings on the vendor rockchip service and Scout boot behavior.
...
|   `-- lidar_harness/                                  # Parametric LD19 harness model and measurement worksheet.
|       |-- README.md                                   # Measurement worksheet, print settings, and post-install steps.
|       `-- scout_ld19_harness.scad                     # Parametric OpenSCAD harness; all hardware dimensions are unverified placeholders.
...
|   |-- cleanup_ros_logs.py                             # Bounds ROS log directories by age and total file size.
|   |-- collect_scout_resource_metrics.sh               # Captures Scout load, memory, swap, disk, and process snapshots.
|   |-- provision_pi_known_hosts.sh                     # Populates Pi-side SSH known hosts without disabling host-key checking.
...
|   |   |-- network_validation.py                       # DNS, TCP, and SSH reachability validation for configured hosts.
|   |   |-- notifications.py                            # Slack payload builders for mission, preflight, and resource alerts.
|   |   |-- paths.py                                    # Common repository path constants.
|   |   |-- redaction.py                                # Strips webhook URLs and other secrets from reports and artifacts.
...
|   |-- companion_notifications.py                      # Python 2 Slack payload builders shared by companion mission reporting.
...
|   |-- ld19_protocol.py                                # Reusable LD19 packet parsing, CRC validation, and scan assembly helpers.
...
|   |-- auto-scout-companion-resource-alert.service     # Example companion critical resource alert unit.
|   |-- auto-scout-companion-resource-alert.timer       # Example timer for periodic companion resource alerts.
...
|   |-- auto-scout-scout-resource-metrics.service       # Example Scout resource metrics collection unit.
|   |-- auto-scout-scout-resource-metrics.timer         # Example timer for periodic Scout resource metrics.
|   |-- auto-scout-scout-runtime.service                # Example Scout systemd unit.
|   |-- auto-scout-scout-swap-setup.service             # Example one-shot setup unit for persistent Scout swap.
|   `-- userdata-auto\x2dscout-auto\x2dscout.swap.swap  # Example systemd swap unit for /userdata/auto-scout/auto-scout.swap.
...
|   |-- test_companion_notifications.py                 # Unit tests for companion Slack payload construction.
|   |-- test_ld19_protocol.py                           # Unit tests for LD19 packet parsing, CRC rejection, and scan assembly.
...
|   |-- test_resource_alerts.py                         # Unit tests for resource alert thresholds and cooldown behavior.
|   |-- test_ros_log_cleanup.py                         # Unit tests for ROS log retention, size accounting, and pruning.
|   |-- test_scout_battery_dock_guard.py                # Unit tests for the low-battery docking guard state machine.
|   |-- test_scout_navigation_controller.py             # Unit tests for the companion smoke-loop mission controller.
|   |-- test_scout_odom_bridge.py                       # Unit tests for vendor odometry normalization and TF publishing.
|   |-- test_scout_safety_filter.py                     # Unit tests for ToF, scan, and battery-guard command-gating decisions.
|   |-- test_scout_sensor_bridges.py                    # Unit tests for ToF and IMU vendor topic normalization.
...
|   `-- test_vendor_jpg_bridge.py                       # Unit tests for vendor JPG frame decoding and lazy subscription.
...
Generated local directories (gitignored, but present in a working checkout):
...
# Run the canonical real-world proof mission after the LD19/nav stack is re-enabled
