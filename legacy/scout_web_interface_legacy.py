#!/usr/bin/env python3
"""
Web dashboard for monitoring and commanding the Scout deployment.

This interface is intended to run on the companion host where the rest of the
navigation stack and stored mission data are easier to manage.
"""
...
class ScoutWebInterface:
    ...
    def __init__(self):
        ...
        self.port = rospy.get_param('~port', self.config.get('web', {}).get('port', 8080))
        ...
        self.scan_sub = rospy.Subscriber(
            self.topic_config.get('lidar_scan', '/scan'),
            LaserScan,
            self.scan_callback,
        )
        ...
        rospy.loginfo(
            "Scout web interface initialized (config: {}, templates: {})".format(
...
def run_flask_app():
    ...
    socketio.run(app, host='0.0.0.0', port=interface.port, debug=False)
...
def main():
    ...
    try:
        ...
        rospy.loginfo("Scout web interface running on http://0.0.0.0:{}".format(interface.port))
